...
def session_new(
    ...
    if not _hook_is_loaded():
        console.print(
            "\n  [dim]Tip: Enable shell hook to auto-record pentest tools:[/dim]"
            '\n  [cyan]eval "$(capo session hook)"[/cyan]'
        )
...
def session_use(
    ...
    if not _hook_is_loaded():
        console.print(
            "  [dim]Tip: eval \"$(capo session hook)\" to auto-record pentest tools[/dim]"
        )
...
def session_log(
    ...
    source: str = typer.Option("manual", "--source", "-s", help="Source tag", hidden=True),
):
    ...
    cmd_id = session_db.record_command(
        tool=tool, command=command, source=source,
    )
    if source != "shell":
...
_HOOK_TEMPLATE = r"""
# ── Capo Session Hook ──────────────────────────────────────
# Auto-records pentest tool commands to the active session.
# Generated by: capo session hook
# ────────────────────────────────────────────────────────────

# Idempotency guard
if [ -n "$_CAPO_HOOK_LOADED" ]; then
    return 0 2>/dev/null || true
fi
export _CAPO_HOOK_LOADED=1

_CAPO_MARKER="%%MARKER%%"

# Known pentest tools (hash set for O(1) lookup)
%%TOOLS_DECL%%

_capo_extract_tool() {
    local cmd="$1"
    # Strip common prefixes: sudo, proxychains, stdbuf, time, nice
    cmd=$(echo "$cmd" | sed -E 's/^(sudo|proxychains4?|proxychains|stdbuf [^ ]+|time|nice -n [0-9]+) +//')
    # Get the first word, then basename
    echo "$cmd" | awk '{print $1}' | xargs basename 2>/dev/null
}

_capo_should_record() {
    [ -s "$_CAPO_MARKER" ] || return 1
    return 0
}
...
if [ -n "$BASH_VERSION" ]; then
    _capo_last_recorded=""
    _capo_cmd_start=$SECONDS

    _capo_debug_timer() {
        _capo_cmd_start=$SECONDS
    }
    trap '_capo_debug_timer' DEBUG

    _capo_prompt_hook() {
        local exit_code=$?
        _capo_should_record || return

        local last_cmd
        last_cmd=$(HISTTIMEFORMAT= history 1 | sed 's/^[ ]*[0-9]*[ ]*//')
        [ "$last_cmd" = "$_capo_last_recorded" ] && return
        _capo_last_recorded="$last_cmd"

        # Skip capo commands (already recorded by wrappers)
        case "$last_cmd" in capo\ *) return ;; esac

        local tool
        tool=$(_capo_extract_tool "$last_cmd")
        [ -z "$tool" ] && return

        if [ ${_CAPO_TOOLS["$tool"]+x} ]; then
            capo session log "$last_cmd" --tool "$tool" --source shell &>/dev/null &
            disown 2>/dev/null
        fi
    }
    PROMPT_COMMAND="_capo_prompt_hook;${PROMPT_COMMAND:-}"
...
# ── Zsh hooks ──────────────────────────────────────────────
    ...
    _capo_precmd() {
        local exit_code=$?
        _capo_should_record || return
        [ -z "$_capo_preexec_cmd" ] && return

        local cmd="$_capo_preexec_cmd"
        _capo_preexec_cmd=""

        case "$cmd" in capo\ *) return ;; esac

        local tool
        tool=$(_capo_extract_tool "$cmd")
        [ -z "$tool" ] && return

        if (( ${+_CAPO_TOOLS[$tool]} )); then
            capo session log "$cmd" --tool "$tool" --source shell &>/dev/null &
            disown 2>/dev/null
        fi
    }

    autoload -Uz add-zsh-hook
    add-zsh-hook preexec _capo_preexec
    add-zsh-hook precmd _capo_precmd
fi
""".lstrip()


@session_app.command("hook")
def session_hook():
    """Output shell hook script for auto-recording pentest tool commands.
    ...
    script = (
        _HOOK_TEMPLATE
        .replace("%%MARKER%%", str(CURRENT_SESSION_FILE))
        .replace("%%TOOLS_DECL%%", tools_decl)
        .replace("%%TOOLS_DECL_ZSH%%", tools_decl_zsh)
    )

    # Raw stdout — no Rich formatting
